...
///
/// Готовность блока определяет только сама страница: `ready` — показываем, `empty` — показывать
/// нечего. Навигация судит исключительно о своём — о том, что загрузка не состоялась; молчащая
/// страница готовой не становится, её добьёт таймаут контейнера. Единственное исключение —
/// отладочная подмена готовности (`EmbeddedBlockReadinessOverrides`) для страниц, которые
/// веб-контракт ещё не умеют.
///
/// Высоту контейнера назначает хост при создании блока: высота из сообщений страницы на вёрстку
/// не влияет, но нулевая в `ready` по-прежнему значит, что страница сломана.
///
/// Экземпляр принадлежит одному контейнеру — ничего общего между контейнерами здесь нет, это и
/// делает возможными несколько независимых блоков. `start()` и `stop()` повторяют видимость
/// контейнера и могут вызываться по кругу; после `stop()` провайдер обязан молчать до следующего
/// `start()` — на это опирается контейнер, когда сворачивает просроченный блок.
///
/// Уход из окна не выбрасывает уже отрендеренную страницу: возврат блока в окно показывает её
/// снова, без сети и без шиммера. Обновить содержимое можно только явно — через `reload()`.
final class EmbeddedBlockWebViewProvider {
    ...
    /// Вью с контентом блока. Контейнер читает её, когда состояние стало `.ready`, и растягивает
    /// по своим краям.
    ...
    ///
    /// Путь тот же, что и у первого запуска, поэтому плейсхолдер, таймаут контейнера и события
    /// хосту работают одинаково: второй ветки жизненного цикла у блока нет.
    func reload() {
    ...
    /// Показан ли блок сейчас — то есть может ли пользователь вообще что-то в нём нажать. Пока
    /// исхода нет, страница ещё грузится, и её сообщения относятся к живому блоку.
    private var isShown: Bool {
        outcome == nil || outcome == .ready
    }

    /// Загруженный документ сам по себе ничего не значит: показывать блок по нему можно только со
    /// включённой отладочной подменой, пока страница не умеет присылать `ready`. Своё `ready`
    /// страницы сильнее — если оно уже пришло, здесь делать нечего.
