...
final class TransparentWebView: UIView {
    ...
    private func createWKWebView() {
        ...
        var mindboxObjectString = "{}"
        if let jsonData = try? JSONSerialization.data(withJSONObject: mindboxParams),
           let jsonString = String(data: jsonData, encoding: .utf8) {
            mindboxObjectString = jsonString
        }
        ...
        let jsObserver: String = """
             ...
             window.sdkBridgeParams = \(mindboxObjectString);
             
             window.SdkBridge = {
                 receiveParam: function(paramName) {
                     return window.sdkBridgeParams[paramName];
                 }
             };
         """
