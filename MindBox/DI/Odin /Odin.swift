...
public class Odin: DependencyInjectionContainer {
    ...
    public typealias TypeResolver = (DependencyInjectionContainer) -> Any
    ...
    ///      return dependency
    ///    }
    public func register<D>(_ resolver: @escaping (DependencyInjectionContainer) -> D) {
        typeResolvers[key(D.self)] = resolver
    }

    public func registerInContainer<D>(_ resolver: @escaping (DependencyInjectionContainer) -> D) {
        container[key(D.self)] = resolver(self)
    }
    ...
    public func resolve<D>() -> D? {
        ...
        if container.keys.contains(key(D.self)),
           let instance = container[key(D.self)] as? D {
            return instance
        }

        return typeResolvers[key(D.self)]?(sself) as? D ??
            parentContainer?.resolve()
    }
}
