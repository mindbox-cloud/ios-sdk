...
struct InappShowBudgetTests {
    ...
    @Test("A missing limit admits every show")
    func missingLimitAdmitsEverything() {
        setLimits(session: nil, day: nil)
        SessionTemporaryStorage.shared.sessionShownInApps = ["a", "b"]
        storage.shownDatesByInApp = ["a": [clock.now], "b": [clock.now]]
    ...
    /// The config carries the interval as a TimeSpan with a fractional part, and Android compares it in milliseconds.
    @Test("A sub-second interval is still an interval")
    func subSecondIntervalIsEnforced() {
        setLimits(interval: "00:00:00.500")
        storage.lastInappStateChangeDate = clock.now.addingTimeInterval(-0.2)

        #expect(!reserve(.overlay("a"), "a"))

        storage.lastInappStateChangeDate = clock.now.addingTimeInterval(-0.6)
