...
class InappTTLTests: XCTestCase {
    ...
    func testNeedResetInapps_WithTTL_Exceeds() throws {
        ...
        let settings = Settings(operations: nil, ttl: .init(inapps: "01:00:00"), slidingExpiration: nil, inapp: nil, featureToggles: nil)
    ...
    func testNeedResetInapps_WithTTL_NotExceeded() throws {
        ...
        let settings = Settings(operations: nil, ttl: .init(inapps: "00:00:02"), slidingExpiration: nil, inapp: nil, featureToggles: nil)
        let config = ConfigResponse(settings: settings)
        let result = service.needResetInapps(config: config)
    ...
    func testNeedResetInapps_WithoutTTL() throws {
        ...
        let settings = Settings(operations: nil, ttl: nil, slidingExpiration: nil, inapp: nil, featureToggles: nil)
        let config = ConfigResponse(settings: settings)
        let result = service.needResetInapps(config: config)
    ...
    func testNeedResetInapps_WithTTLHalfMinutesAgo_NotExceeded() throws {
        ...
        let settings = Settings(operations: nil, ttl: .init(inapps: "00:01:00"), slidingExpiration: nil, inapp: nil, featureToggles: nil)
        let config = ConfigResponse(settings: settings)
        let result = service.needResetInapps(config: config)
        XCTAssertFalse(result, "Inapps не должны быть сброшены, так как время TTL еще не истекло.")
    }
    ...
    func testNeedResetInapps_WithTTLOneDayAgo_NotExceeded() throws {
        ...
        let settings = Settings(operations: nil, ttl: .init(inapps: "2.00:00:00"), slidingExpiration: nil, inapp: nil, featureToggles: nil)
        let config = ConfigResponse(settings: settings)
        let result = service.needResetInapps(config: config)
        XCTAssertFalse(result, "Inapps не должны быть сброшены, так как время TTL еще не истекло.")
    }
    ...
    func testNeedResetInapps_WithMinusOneDayTTL_NotExceeded() throws {
        ...
        let settings = Settings(operations: nil, ttl: .init(inapps: "-1.00:00:00"), slidingExpiration: nil, inapp: nil, featureToggles: nil)
        let config = ConfigResponse(settings: settings)
        let result = service.needResetInapps(config: config)
        XCTAssertFalse(result, "Inapps не должны быть сброшены, так как время TTL еще не истекло.")
    }
